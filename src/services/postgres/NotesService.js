...
const AuthorizationError = require('../../exceptions/AuthorizationErros');
const { mapDBToModel } = require('../../utils');

class NotesService {
  constructor(collaborationsService, cacheService) {
    this._pool = new Pool();
    this._collaborationnService = collaborationsService;
  ...
  async addNote({
    ...
    // console.log(`owner nya ini: ${owner}`);
    const query = {
  ...
  async verifyNoteAccess(noteId, userId) {
    try {
      ...
      try {
        await this._collaborationnService.verifyCollaborator(noteId, userId);
      } catch {
        throw error;
      }
    }
